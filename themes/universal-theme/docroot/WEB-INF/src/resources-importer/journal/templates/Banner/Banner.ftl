<#if image?? && image.data?has_content>
...
<#if opacity?? && opacity.data?has_content>
...
<#if position?? && position.data?has_content>
	<#assign position_class = position.data />
<#else>
	<#assign position_class = "" />
</#if>

<div class="align-center block-container justify-center main-banner ${opacity_overlay} ${position_class}" ${style}>
	<div class="block ${font_color} main-banner-content max-med">
		<#if heading?? && heading.data?has_content>
			<div class="page-heading">
				<h1>${heading.data}</h1>

				<#if sub_heading?? && sub_heading.data?has_content>
